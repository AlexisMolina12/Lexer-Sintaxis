...
ESTADOS_POSIBLES = [
    ('TOKEN_ESPACIO',          automata_espacio),          # se descarta
    ('TOKEN_AND',              automata_and),
...
def lexer(codigo_fuente):
    ...
    while pos_actual < len(codigo_fuente):
        comienzo_lexema = pos_actual # el siguiente lexema comienza en la posicion actual(se comienza desde el ultimo caracter del ultimo lexema reconocido)
        ...
        while not var_aux_estado_trampa and pos_actual < len(codigo_fuente)+1:
            ...
            posibles_token_mas_caracter = [] #Se reinicia la lista auxiliar que contenian lexemas aceptados 
            for (un_token, automata) in ESTADOS_POSIBLES: #en este for se simulan todos los automatas y sigue ciclando el while de arriba
                simulacion_automata = automata(lexema)
                if simulacion_automata == ESTADO_FINAL and un_token != 'TOKEN_ESPACIO': #Descartamos el token espacio
                    posibles_token_mas_caracter.append(un_token)
                    var_aux_estado_trampa = False
        ...
        un_token = posibles_tokens[0] #Se toma el primer elemento por convencion. Esto sucede si mas de un automata acepta el mismo lexema 
                                    ...
                                    #mientras que el lexema tenga la posibilidad de avanzar, ya sea si esta en un estado final o no, 
